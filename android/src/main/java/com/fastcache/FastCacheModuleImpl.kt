...
/**
 * Shared implementation for both legacy and New Architecture
 */
object FastCacheModuleImpl {
    
    fun preload(reactContext: ReactApplicationContext, sources: ReadableArray) {
    ...
    fun clearDiskCache(reactContext: ReactApplicationContext, promise: Promise) {
        try {
            Thread {
                try {
                    val context = reactContext.currentActivity ?: reactContext
    ...
    fun getCacheSize(reactContext: ReactApplicationContext, promise: Promise) {
        try {
            Thread {
                try {
    ...
    fun getCachePath(reactContext: ReactApplicationContext, promise: Promise) {
        try {
            val cacheDir = reactContext.cacheDir
            val glideCache = File(cacheDir, "image_manager_disk_cache")
